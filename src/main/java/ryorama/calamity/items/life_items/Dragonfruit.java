...
public class Dragonfruit extends ItemBaseTC implements ILeftClickableItem {
   public Dragonfruit() {
      this.setTranslationKey("dragon_fruit");
      this.stackSize = 1;
   ...
   public void onLeftClick(EntityPlayer player, ItemStack stack) {
      ...
      useDragonfruit(player, stack);
   }

   public static boolean useDragonfruit(EntityPlayer player, ItemStack stack) {
      if ((float) PlayerDataTC.get(player).health().additionalHealthAddedByFruits >= 100.0F) {
         PlayerDataTC pd = PlayerDataTC.get(player);
         boolean did = ((LifeExpansionManager)pd.getData(LifeExpansionManager.class)).addDragonfruit();
